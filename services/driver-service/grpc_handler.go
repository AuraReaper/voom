...
func (h *grpcHandler) GetDrivers(ctx context.Context, req *pb.GetDriversRequest) (*pb.GetDriversResponse, error) {
	drivers, err := h.Service.GetDrivers()
	if err != nil {
		return nil, status.Errorf(codes.Internal, "failed to get drivers")
	}
